...
2. Frontend: CSS solo da file statici con hash (uploads/blocky/); MAI wp_add_inline_style per utility CSS; MAI Google Fonts by default.
3. Documento JSON: ogni modifica = doc-version + migrazione esplicita; parser unico PHP valida save/REST/agent.
4. Stessa versione motore CSS ovunque (binario CLI = fonte canonica; compile JS in browser solo preview).
