...
> **Aggiornamento 2026-09-16 (Phase B, wp.org review)**: binario CLI server-side ELIMINATO (violazione "binary download/exec" rilevata in review). Motore unico = pacchetto npm `tailwindcss`, compilato nel browser admin al salvataggio (fase UI "Compiling styles…"). L3 = un file statico con hash PER PAGINA in `uploads/blocky/page-{id}-{md5}.css` (meta `_blocky_css_file`), con catena di fallback: stringa meta `_blocky_css_cache` → vocabolario statico `engine/assets/css/vocabulary-fallback.css` (pagine migrate mai ricompilate). Precedente in directory: WindPress (compila via WASM nel browser admin). Lezione registrata in docs/research/11.
...
- Se un giorno arriverà il supporto temi terzi: il _theme bridge_ entra come **input extra del compilatore** (preflight scoped + mapping theme.json), senza toccare i 3 layer. Architettura pronta, feature rimandata.
...
Nota d'ordinamento: P1 prima di P2 perché il Control Kit _definisce_ il closed-set definitivo della safelist — ma lo schema dei descriptor si progetta **durante** P1 (il closed-set di S1/P1 nasce provvisoriamente dai cataloghi attuali; P2 lo congela).
