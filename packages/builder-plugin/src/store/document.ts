...
import { collectMegaMenuCustomSlotNames, defaultMegaMenuItems, isMegaMenuCustomSlotName, normalizeMegaMenuItems } from '../megaMenu/items';
import { defaultOverlayId, normalizeManagedOverlayIds, overlayIdForNode, supportsManagedOverlayId, supportsOverlaySlots } from '../overlays/identity';
import type { BlockInsertPreset, BuilderDocument, BuilderNode } from '../sdk/types';
import { compileFrontendPageCss, compileFrontendPageCssFromHtml } from '../tailwind/pageCssCompiler';
import { t } from '../i18n';
...
export type PageStarterId = 'page' | 'landing' | 'base-template' | 'header' | 'footer' | 'menu' | 'sidebar';
...
interface DocumentState {
  ...
  insertBlock:   (type: string, targetId?: string, position?: InsertPosition, slotName?: string, preset?: BlockInsertPreset, gridPlacement?: GridPlacement) => void;
  removeBlock:   (id: string) => void;
  duplicateBlock:(id: string) => void;
  moveBlock:     (id: string, direction: MoveDirection) => void;
  moveBlockTo:   (id: string, targetId: string, position: InsertPosition, slotName?: string, gridPlacement?: GridPlacement) => void;
...
function defaultPropsForType(type: string): Record<string, unknown> {
  const defaults: Record<string, Record<string, unknown>> = {
    ...
    'bky/section':   {
      paddingY: 'lg', paddingX: 'base', background: 'transparent', textColor: 'base', fullWidth: false,
      contentWidth: 'container', minHeight: 'auto', verticalAlign: 'start', horizontalAlign: 'start', gap: 'base',
      border: 'none', radius: 'none', shadow: 'none', overflow: 'visible', customClass: '',
    },
    'bky/rows':      {
      count: 3,
      gap: 'base',
    ...
    'bky/container': { maxWidth: 'base', align: 'center', padding: 'none', background: 'transparent', radius: 'none' },
    'bky/columns':   {
      count: 2,
      gap: 'base',
      stackAt: 'md',
    ...
    'bky/card':      { padding: 'base', background: 'elevated', border: true, shadow: 'sm', overflow: 'hidden' },
    ...
    'bky/icon':      { icon: '★', href: '', target: '_self', size: 'base', tone: 'accent', background: 'transparent', border: 'none', radius: 'full', shadow: 'none', align: 'start' },
    'bky/icon-box':  { icon: '✨', title: 'Icon box', text: 'Describe this feature or benefit.', href: '', target: '_self', layout: 'vertical', align: 'start', gap: 'base', iconSize: 'base', iconTone: 'accent', background: 'transparent', textColor: 'inherit', border: 'none', radius: 'lg', shadow: 'none' },
    'bky/icon-list': { icon: '✓', items: 'First item\nSecond item\nThird item', gap: 'base', tone: 'base' },
    'bky/image-box': { attachmentId: 0, title: 'Image box', text: 'Describe this image or linked content.', href: '', target: '_self', layout: 'vertical', align: 'start', gap: 'base', imageWidth: 'full', rounded: 'base', aspectRatio: 'wide', background: 'transparent', textColor: 'inherit', border: 'none', radius: 'lg', shadow: 'none' },
    'bky/alert':     { title: 'Heads up', message: 'Share an important update or status message.', icon: 'ℹ', tone: 'info', radius: 'lg' },
    'bky/progress-bar': { label: 'Progress', value: 65, showLabel: true, striped: false, size: 'base', tone: 'accent', radius: 'base' },
    'bky/counter':   { prefix: '', value: 128, suffix: '+', label: 'Satisfied customers', align: 'start', size: 'base', tone: 'accent' },
    'bky/star-rating': { value: 4.5, max: 5, label: '4.5 average rating', size: 'base', tone: 'gold', align: 'start' },
    'bky/anchor':    { anchorId: 'section-anchor', label: 'Section anchor' },
    'bky/social-icons': { items: 'facebook|https://facebook.com\ninstagram|https://instagram.com\nlinkedin|https://linkedin.com', layout: 'row', gap: 'base', align: 'start', size: 'base', tone: 'brand', radius: 'full' },
    'bky/share-buttons': { networks: 'facebook,x,linkedin', title: 'Share this page', gap: 'base', align: 'start', size: 'base', tone: 'neutral', radius: 'button' },
    'bky/search-form': { placeholder: 'Search…', buttonLabel: 'Search', layout: 'inline', gap: 'base', buttonTone: 'primary', radius: 'button' },
    'bky/nav-menu': { menuLocation: '', items: 'Home|/\nAbout|/about\nContact|/contact', layout: 'row', gap: 'base', align: 'start' },
    ...
    'bky/featured-posts': { title: 'Featured posts', perPage: 3, layout: 'grid', showExcerpt: true, stickyOnly: true },
    'bky/taxonomy-list': { title: 'Browse topics', taxonomy: 'category', layout: 'pills', showCount: true, limit: 8 },
    ...
    'bky/image-carousel': { items: '', slidesVisible: '1', gap: 'base', aspectRatio: 'video', showCaptions: true, autoPlay: false, interval: 5 },
    'bky/content-carousel': { items: 'Slide 1|Highlight your offer|Learn more|#\nSlide 2|Guide visitors to the next step|Contact us|#', slidesVisible: '1', gap: 'base', align: 'start', tone: 'surface', autoPlay: false, interval: 5 },
    'bky/slider': { items: 'Slide 1|Highlight your offer|Learn more|#\nSlide 2|Guide visitors to the next step|Contact us|#', slidesVisible: '1', gap: 'base', align: 'start', tone: 'surface', autoPlay: false, interval: 5 },
    'bky/call-to-action': { attachmentId: 0, eyebrow: 'Limited release', title: 'Launch a focused campaign with Blocky', text: 'Combine persuasive copy, supporting media, and clear next steps inside a reusable CTA section.', primaryLabel: 'Start now', primaryUrl: '#', secondaryLabel: 'Book a demo', secondaryUrl: '#', layout: 'split', align: 'start', mediaPosition: 'end', tone: 'surface', padding: 'lg' },
    'bky/testimonial': { attachmentId: 0, quote: 'Blocky helped us ship faster without losing layout control.', author: 'Alex Morgan', role: 'Product Marketing Lead', layout: 'card', tone: 'surface' },
    'bky/price-table': { title: 'Growth', subtitle: 'For teams ready to scale.', price: '49', currency: '€', cadence: '/mo', features: 'Unlimited sections\nShared design tokens\nPriority support', buttonLabel: 'Choose plan', buttonUrl: '#', featured: false, align: 'start', tone: 'surface' },
    'bky/price-list': { items: 'Strategy Session|60 minute workshop|€120\nImplementation Sprint|Landing page setup|€900', showDividers: true, tone: 'surface' },
    'bky/flip-box': { frontTitle: 'Feature teaser', frontText: 'Keep the front side concise and visual.', backTitle: 'Reveal the full pitch', backText: 'Use the back side for detail, proof, or the next action.', buttonLabel: 'Learn more', buttonUrl: '#', height: 'base', verticalAlign: 'end', tone: 'surface' },
    'bky/hotspot': { attachmentId: 0, points: 'Hero area|28|35|Point out the main value proposition.\nCall to action|70|62|Use a second marker for the next step.', tone: 'surface' },
    'bky/before-after-slider': { beforeAttachmentId: 0, afterAttachmentId: 0, startingPoint: 50, aspectRatio: 'video' },
    'bky/countdown': { targetDate: '2030-01-01T00:00:00', showLabels: true, layout: 'grid', tone: 'surface' },
    'bky/animated-headline': { prefix: 'Build', words: 'faster\nsmarter\nwith Blocky', suffix: 'pages', effect: 'rotate', interval: 3, align: 'start', tone: 'accent' },
    'bky/marquee': { items: 'Launch faster\nReusable sections\nWordPress native\nTailwind-first', speed: 20, direction: 'left', pauseOnHover: true, tone: 'surface' },
    'bky/lottie': { url: '', poster: 0, autoplay: true, loop: true, speed: 1, aspectRatio: 'square' },
    'bky/embed-google-maps': { provider: 'google', query: 'Milan, Italy', latitude: '', longitude: '', zoom: 12, title: 'Map embed', allowFullscreen: true, height: 'base', aspectRatio: 'wide' },
    'bky/embed-iframe': { url: '', title: 'Embedded iframe', sandbox: 'allow-scripts allow-same-origin allow-forms', allow: 'fullscreen; autoplay; clipboard-read; clipboard-write', lazyLoad: true, allowFullscreen: true, height: 'base', aspectRatio: 'video' },
    'bky/code-highlight': { code: "const hello = 'Blocky';\nconsole.log(hello);", language: 'javascript', caption: '', showLineNumbers: true, tone: 'contrast' },
    'bky/mega-menu': { menuLocation: '', items: defaultMegaMenuItems(), openOn: 'hover', columns: '3', panelWidth: 'lg', gap: 'base', align: 'start' },
    'bky/login-form': { formId: '', redirectUrl: '', title: 'Welcome back', buttonLabel: 'Sign in', showRemember: true, showLostPassword: true, showRegisterLink: true, gap: 'base', padding: 'base', background: 'surface', border: 'subtle', radius: 'lg' },
    'bky/register-form': { formId: '', redirectUrl: '', title: 'Create your account', submitLabel: 'Create account', successMessage: 'Your account has been created.', loginAfterRegister: false, gap: 'base', padding: 'base', background: 'surface', border: 'subtle', radius: 'lg' },
    'bky/contact-form': { formId: '', title: "Let's talk", successMessage: 'Thanks, we received your message.', nameLabel: 'Name', emailLabel: 'Email', subjectLabel: 'Subject', messageLabel: 'Message', submitLabel: 'Send message', gap: 'base', padding: 'base', background: 'surface', border: 'subtle', radius: 'lg' },
    'bky/author-box': { title: 'About the author', showAvatar: true, showBio: true, showArchiveLink: true, avatarSize: 'base', layout: 'row', padding: 'base', background: 'surface', border: 'subtle', radius: 'lg' },
    'bky/comments': { title: 'Discussion', perPage: 6, showAvatar: true, showDate: true, order: 'asc' },
    'bky/comment-form': { titleReply: 'Leave a reply', buttonLabel: 'Post comment', showNotes: true },
    'bky/post-navigation': { prevLabel: 'Previous post', nextLabel: 'Next post', layout: 'between', showLabels: true },
    'bky/sitemap': { title: 'Sitemap', includePages: true, includePosts: true, postsPerPage: 6, showDescriptions: false, orderBy: 'menu_order' },
    ...
    'bky/sticky-bar': { title: 'Keep this page within reach', text: 'Pin an important CTA, promo, or status message while visitors scroll.', buttonLabel: 'Get started', buttonUrl: '#', position: 'bottom', showAfter: 15, dismissible: true, tone: 'surface' },
    'bky/back-to-top': { label: 'Back to top', showLabel: false, position: 'right', showAfter: 20, size: 'base', tone: 'accent' },
    'bky/popup': { overlayId: '', title: 'Popup title', description: 'Use this popup for announcements, lead capture, or offers.', placement: 'center', size: 'md', backdrop: 'dim', dismissEsc: true, dismissOutside: true, focusTrap: true, openOn: 'exit-intent', frequency: 'session', closeOn: '', defaultOpen: false },
    'bky/notification-toast': { overlayId: '', title: 'Saved', description: 'Your changes were saved successfully.', placement: 'right', size: 'sm', backdrop: 'none', dismissEsc: true, dismissOutside: false, focusTrap: false, openOn: 'delay:1500', frequency: 'always', closeOn: 'timeout:3000', defaultOpen: false },
    'bky/cookie-banner': { overlayId: '', title: 'We use cookies', description: 'Use cookies to improve the browsing experience and measure content performance.', placement: 'bottom', size: 'xl', backdrop: 'none', dismissEsc: true, dismissOutside: false, focusTrap: false, openOn: 'load', frequency: 'once', closeOn: '', defaultOpen: false, cookieName: 'blocky_cookie_consent', acceptLabel: 'Accept', rejectLabel: 'Reject', cookieDurationDays: 180 },
    'bky/lightbox': { overlayId: '', triggerLabel: 'Open media', mediaUrl: '', mediaType: 'image', caption: '', thumbnailUrl: '', aspectRatio: 'video' },
    'bky/command-palette': { overlayId: '', title: 'Command palette', description: 'Search common actions, routes, or documentation.', placement: 'top', size: 'lg', backdrop: 'blur', dismissEsc: true, dismissOutside: true, focusTrap: true, openOn: 'shortcut:mod+k', frequency: 'always', closeOn: '', defaultOpen: false },
    'bky/tabs': { items: 'Tab 1|First tab content\nTab 2|Second tab content\nTab 3|Third tab content', activeIndex: 0 },
    ...
    'bky/form': { formId: '', action: '', method: 'post', name: '', successMessage: 'Thanks, your submission has been saved.', enctype: 'default', noValidate: false, gap: 'base', align: 'start', padding: 'none', background: 'transparent', border: 'none', radius: 'none', shadow: 'none' },
    'bky/form-field-text': { label: 'Your name', name: 'name', placeholder: 'Enter a value', inputType: 'text', autocomplete: '', helpText: '', required: false, defaultValue: '', width: 'full' },
    'bky/form-field-textarea': { label: 'Message', name: 'message', placeholder: 'Write your message', rows: 4, helpText: '', required: false, defaultValue: '', width: 'full' },
    'bky/form-field-select': { label: 'Topic', name: 'topic', options: 'Support|support\nSales|sales\nPartnerships|partnerships', placeholder: 'Choose an option', helpText: '', required: false, defaultValue: '', width: 'full' },
    'bky/form-field-radio': { label: 'Preferred contact', name: 'contact_preference', options: 'Email|email\nPhone|phone', helpText: '', required: false, defaultValue: 'email', layout: 'stacked', width: 'full' },
    'bky/form-field-checkbox': { label: 'Interests', name: 'interests', options: 'Design|design\nDevelopment|development\nStrategy|strategy', helpText: '', required: false, defaultValue: '', layout: 'stacked', width: 'full' },
    'bky/form-field-date': { label: 'Preferred date', name: 'preferred_date', inputType: 'date', min: '', max: '', helpText: '', required: false, defaultValue: '', width: 'full' },
    'bky/form-field-file': { label: 'Attachment', name: 'attachment', accept: '', helpText: '', multiple: false, required: false, width: 'full' },
    'bky/form-field-hidden': { name: 'source', value: 'builder' },
    'bky/form-field-honeypot': { name: 'website', label: 'Leave this field blank' },
    'bky/form-submit': { label: 'Send message', variant: 'primary', size: 'base', align: 'start', fullWidth: false, radius: 'button', shadow: 'none' },
    'bky/modal': { overlayId: '', title: 'Modal title', description: 'Add content to the modal body slot.', placement: 'center', size: 'md', backdrop: 'dim', dismissEsc: true, dismissOutside: true, focusTrap: true, openOn: '', frequency: 'always', closeOn: '', defaultOpen: false },
    'bky/offcanvas': { overlayId: '', title: 'Offcanvas panel', description: 'Add navigation, filters, or utility content.', placement: 'right', size: 'md', backdrop: 'dim', dismissEsc: true, dismissOutside: true, focusTrap: true, openOn: '', frequency: 'always', closeOn: '', defaultOpen: false },
    'bky/drawer': { overlayId: '', title: 'Drawer', description: 'Add compact actions, filters, or support content.', placement: 'left', size: 'sm', backdrop: 'dim', dismissEsc: true, dismissOutside: true, focusTrap: true, openOn: '', frequency: 'always', closeOn: '', defaultOpen: false },
    'bky/modal-trigger': { label: 'Open overlay', targetOverlayId: '', action: 'overlay.open', variant: 'primary', size: 'base', align: 'start', fullWidth: false, radius: 'button', shadow: 'none' },
    'bky/popover': { overlayId: '', title: 'Popover', description: 'Add short contextual content next to a trigger.', placement: 'bottom', size: 'sm', backdrop: 'none', dismissEsc: true, dismissOutside: true, focusTrap: false, openOn: '', frequency: 'always', closeOn: '', defaultOpen: false },
    'bky/tooltip': { overlayId: '', title: 'Tooltip', description: 'Short helper text for a nearby trigger.', placement: 'top', size: 'sm', backdrop: 'none', dismissEsc: true, dismissOutside: true, focusTrap: false, openOn: '', frequency: 'always', closeOn: '', defaultOpen: false },
    'bky/dialog-confirm': { overlayId: '', title: 'Confirm action', description: 'Review the action before confirming.', placement: 'center', size: 'sm', backdrop: 'dim', dismissEsc: true, dismissOutside: false, focusTrap: true, openOn: '', frequency: 'always', closeOn: '', defaultOpen: false, cancelLabel: 'Cancel', confirmLabel: 'Confirm' },
    ...
    'bky/theme-toggle':     { label: t('document.toggleTheme', 'Toggle Theme'), size: 'md', variant: 'outline' },
  };
...
function collectNodeSubtreeIds(document: BuilderDocument, nodeId: string, target: Set<string>): void {
...
function syncMegaMenuSlots(document: BuilderDocument, node: BuilderNode, nextItems: unknown, pruneRemoved: boolean): void {
  ...
  currentSlotNames.forEach(slotName => {
    ...
    (node.slots[slotName] ?? []).forEach(childId => collectNodeSubtreeIds(document, childId, toDelete));
...
function normalizeMegaMenuStructures(document: BuilderDocument, pruneRemoved: boolean): boolean {
  ...
  Object.values(document.nodes).forEach(node => {
    ...
    const previousSlotNames = JSON.stringify(Object.keys(node.slots).filter(isMegaMenuCustomSlotName).sort());
    syncMegaMenuSlots(document, node, normalizedItems, pruneRemoved);
    const nextSlotNames = JSON.stringify(Object.keys(node.slots).filter(isMegaMenuCustomSlotName).sort());
...
function createNode(type: string, preset?: BlockInsertPreset): BuilderNode {
  ...
  if (supportsManagedOverlayId(type) && String(node.props['overlayId'] ?? '').trim() === '') {
...
function remapOverlayTargets(node: BuilderNode, overlayTargetMap: Map<string, string>): void {
  const targetOverlayId = String(node.props['targetOverlayId'] ?? '').trim();
  ...
  node.props['interactions'] = interactions.map(entry => {
    ...
    if (!action.startsWith('overlay.') || target === '' || !overlayTargetMap.has(target)) {
      return entry;
    }

    return {
      ...entry,
      target: overlayTargetMap.get(target) ?? target,
    };
  });
}
...
function targetIndexForMove(parent: BuilderNode, index: number, childCount: number, direction: MoveDirection): number | null {
...
function orderedStructuredSlotNames(parent: BuilderNode): string[] {
  if (parent.type === 'bky/columns') return ['column-1', 'column-2', 'column-3', 'column-4', 'column-5', 'column-6'];
...
function moveStructuredChild(
  ...
  const movesAcrossSlots = parent.type === 'bky/columns'
...
function moveGridChild(document: BuilderDocument, parent: BuilderNode, slot: string[], nodeId: string, index: number, direction: MoveDirection): boolean {
...
function gridChildPositions(document: BuilderDocument, slot: string[], columns: number): Map<string, GridPosition> {
...
function gridPositionForNode(node: BuilderNode | undefined, index: number, columns: number): GridPosition {
...
function targetGridPosition(position: GridPosition, columns: number, rows: number, direction: MoveDirection): GridPosition | null {
...
function gridRowCount(parent: BuilderNode, childCount: number, positions: Map<string, GridPosition>): number {
...
function applyGridPlacement(document: BuilderDocument, nodeId: string, targetId: string, gridPlacement?: GridPlacement): void {
  ...
  node.props['gridColumnStart'] = boundedInteger(gridPlacement.column, 1, 1, gridColumnCount(parent));
...
export const useDocumentStore = create<DocumentState>()(
  immer((set, get) => ({
    ...
    async loadPost(postId) {
      set(s => { s.isLoading = true; s.postId = postId; });
      try {
        const res  = await apiFetch(`documents/${postId}`);
        const data = await res.json() as { document?: BuilderDocument | null; html?: unknown; post?: unknown };
        ...
        const nextPreviewCss = nextHtml !== '' ? await compileFrontendPageCssFromHtml(nextHtml) : '';
    ...
    async save() {
      ...
      } catch { /* save errors are surfaced by the browser/network panel for now */ }
    },
    ...
    async loadPageLibrary() {
      set(s => { s.isLoadingPages = true; });
      try {
        const res = await apiFetch('documents/library');
        const data = await res.json() as { items?: unknown[] };
        const pages = Array.isArray(data.items)
          ? data.items.map(item => normalizePageRecord(item)).filter((item): item is BuilderPageRecord => item !== null)
          : [];
    ...
    insertBlock(type, targetId, position = 'inside', slotName = 'default', preset, gridPlacement) {
      set(s => {
        ...
        const inserted = insertNodeId(s.document, node.id, target, position, slotName)
          || (target !== s.document.root && insertNodeId(s.document, node.id, s.document.root, 'inside'));
    ...
    updateProps(id, props) {
      set(s => {
        ...
        if (node) {
          ...
          if (node.type === 'bky/mega-menu' && Object.prototype.hasOwnProperty.call(nextProps, 'items') && s.document) {
          ...
          if (s.document && supportsManagedOverlayId(node.type) && Object.prototype.hasOwnProperty.call(props, 'overlayId')) {
...
function emptyDocument(): BuilderDocument {
  ...
  return {
    root:  rootId,
    nodes: {
      [rootId]: { id: rootId, type: 'bky/section', props: defaultPropsForType('bky/section'), slots: { default: [] }, variants: {} },
    },
  };
}
...
function starterDocument(starter: PageStarterId = 'page'): BuilderDocument {
  ...
  switch (starter) {
    case 'base-template': {
      ...
      addStarterNode(document, 'bky/wp-template-part', headerSectionId, 'default', { props: { postId: 0 } });
      ...
      addStarterNode(document, 'bky/wp-post-title', mainSectionId, 'default', { props: { level: 1 } });
      ...
      addStarterNode(document, 'bky/wp-template-part', footerSectionId, 'default', { props: { postId: 0 } });
      break;
    }
    case 'landing': {
      root.props = {
      ...
      addStarterNode(document, 'bky/heading', rootId, 'default', { props: { level: 1, text: 'Landing page', align: 'center' } });
      addStarterNode(document, 'bky/text', rootId, 'default', { props: { content: 'Presenta il messaggio principale qui e guida l’utente verso una call to action chiara.', align: 'center' } });
      addStarterNode(document, 'bky/button', rootId, 'default', { props: { label: 'Start here', href: '#', align: 'center' } });
      break;
    }
    case 'header': {
      root.props = {
      ...
      addStarterNode(document, 'bky/heading', containerId, 'default', { props: { level: 3, text: 'Brand', align: 'start' } });
      ...
      ['Home', 'Servizi', 'Chi siamo', 'Contatti'].forEach(label => {
        addStarterNode(document, 'bky/button', menuId, 'default', { props: { label, href: '#', variant: 'outline' } });
      });
      break;
    }
    case 'footer': {
      ...
      addStarterNode(document, 'bky/text', containerId, 'default', { props: { content: '© 2026 Your Company. Tutti i diritti riservati.' } });
      ...
      ['Privacy', 'Termini', 'Contatti'].forEach(label => {
        addStarterNode(document, 'bky/button', linksId, 'default', { props: { label, href: '#', variant: 'ghost' } });
      });
      break;
    }
    case 'menu': {
    ...
    case 'sidebar': {
      ...
      addStarterNode(document, 'bky/heading', columnsId, 'column-1', { props: { level: 2, text: t('starter.mainContent', 'Main content') } });
      addStarterNode(document, 'bky/text', columnsId, 'column-1', { props: { content: t('starter.mainContentDescription', 'Use this column for the main content of the page.') } });
      addStarterNode(document, 'bky/list', columnsId, 'column-2', { props: { ordered: false, items: t('starter.keyPoints', 'Key point one\nKey point two\nKey point three') } });
      const cardId = addStarterNode(document, 'bky/card', columnsId, 'column-3', { props: { padding: 'base', background: 'elevated', border: true, shadow: 'sm' } });
      addStarterNode(document, 'bky/heading', cardId, 'default', { props: { level: 3, text: t('starter.sidebar', 'Sidebar') } });
      addStarterNode(document, 'bky/text', cardId, 'default', { props: { content: t('starter.sidebarDescription', 'Add CTAs, quick links, or secondary information here.') } });
      addStarterNode(document, 'bky/button', cardId, 'default', { props: { label: t('starter.callToAction', 'Call to action'), href: '#', align: 'start' } });
      break;
    }
    case 'page':
    default: {
      ...
      addStarterNode(document, 'bky/heading', rootId, 'default', { props: { level: 1, text: t('starter.newPage', 'New page') } });
      addStarterNode(document, 'bky/text', rootId, 'default', { props: { content: t('starter.newPageDescription', 'Start here: add sections, content, and layout to build the page.') } });
      break;
    }
  }
...
function defaultTitleForStarter(starter: PageStarterId = 'page'): string {
  switch (starter) {
    ...
    case 'base-template': return 'Base Template';
    case 'header': return 'Header Layout';
    case 'footer': return 'Footer Layout';
    case 'menu': return 'Navigation Menu';
    case 'sidebar': return 'Sidebar Layout';
    case 'page':
    default:
...
function normalizePageRecord(value: unknown): BuilderPageRecord | null {
  ...
  return {
    id,
    title: typeof record.title === 'string' && record.title.trim() !== '' ? record.title : '(Untitled)',
...
function upsertPageRecord(items: BuilderPageRecord[], nextItem: BuilderPageRecord): BuilderPageRecord[] {
...
async function saveDocumentResponseFromResponse(res: Response): Promise<{ html: string; classCandidates: string[] } | null> {
  if (!res.ok) return null;
  ...
  const classCandidates = Array.isArray(data.classCandidates)
    ? data.classCandidates.filter((value): value is string => typeof value === 'string' && value !== '')
    : [];
...
async function compileAndPersistPageCss(postId: number, classCandidates: string[]): Promise<string> {
