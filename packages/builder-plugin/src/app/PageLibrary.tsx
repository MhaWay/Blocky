...
import { type BuilderPageRecord, type CreatePostOptions, useDocumentStore } from '../store/document';
...
const PAGE_STARTERS: Array<{
  ...
  { id: 'page', label: t('library.pageStarterLabel', 'Page'), description: t('library.pageStarterDescription', 'A standard page to build freely.'), icon: 'P', title: t('library.pageStarterTitle', 'New Blocky Page') },
  { id: 'landing', label: t('library.landingStarterLabel', 'Landing'), description: t('library.landingStarterDescription', 'Hero, central content, and a call to action.'), icon: 'L', title: t('library.landingStarterTitle', 'Landing Page') },
];
...
const TEMPLATE_STARTERS: Array<{
  ...
  { id: 'base-template', label: t('library.baseTemplateLabel', 'Base Template'), description: t('library.baseTemplateDescription', 'Primary template with header area, dynamic page content, and footer.'), icon: 'T', title: t('library.baseTemplateTitle', 'Base Template') },
  { id: 'header', label: t('library.headerStarterLabel', 'Header'), description: t('library.headerStarterDescription', 'Starter for a top bar with branding and navigation.'), icon: 'H', title: t('library.headerStarterTitle', 'Header Layout') },
  { id: 'footer', label: t('library.footerStarterLabel', 'Footer'), description: t('library.footerStarterDescription', 'Starter for a footer with links and notes.'), icon: 'F', title: t('library.footerStarterTitle', 'Footer Layout') },
  { id: 'menu', label: t('library.menuStarterLabel', 'Menu'), description: t('library.menuStarterDescription', 'Quick navigation with pre-aligned items.'), icon: 'M', title: t('library.menuStarterTitle', 'Navigation Menu') },
  { id: 'sidebar', label: t('library.sidebarStarterLabel', 'Sidebar'), description: t('library.sidebarStarterDescription', 'Layout with main content and a side column.'), icon: 'S', title: t('library.sidebarStarterTitle', 'Sidebar Layout') },
];
...
export const PageLibrary: FunctionComponent = () => {
  ...
  const [contextMenu, setContextMenu] = useState<{ page: BuilderPageRecord; x: number; y: number } | null>(null);
    ...
    <div class="mx-auto flex h-full w-full max-w-7xl flex-col gap-8 px-6 py-8">
      <section class="rounded-card border border-border-subtle bg-surface-elevated p-6 shadow-sm">
        <div class="mb-5 flex items-center justify-between gap-4">
          <div>
            <p class="text-xs font-semibold uppercase tracking-[0.18em] text-text-faint">{t('library.create', 'Create')}</p>
            <h2 class="mt-2 font-display text-2xl font-bold text-text-base">{t('library.newLayout', 'New Layout')}</h2>
            <p class="mt-2 max-w-2xl text-sm text-text-muted">{t('library.newLayoutDescription', 'Choose the type of page to generate. Each card creates a new draft page with a different starter, ready to refine in Blocky.')}</p>
          </div>
        </div>
      ...
      <section class="rounded-card border border-border-subtle bg-surface-elevated p-6 shadow-sm">
        <div class="mb-5 flex items-center justify-between gap-4">
          <div>
            <p class="text-xs font-semibold uppercase tracking-[0.18em] text-text-faint">{t('library.templates', 'Templates')}</p>
            <h2 class="mt-2 font-display text-2xl font-bold text-text-base">{t('library.newTemplates', 'New Templates')}</h2>
            <p class="mt-2 max-w-2xl text-sm text-text-muted">{t('library.newTemplatesDescription', 'Create reusable parts or a full base template to connect to pages through Blocky WordPress blocks.')}</p>
          </div>
        </div>
      ...
      <section class="flex flex-1 flex-col rounded-card border border-border-subtle bg-surface-elevated p-6 shadow-sm">
        <div class="mb-5 flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <h2 class="mt-2 font-display text-2xl font-bold text-text-base">{t('library.allPages', 'All Pages')}</h2>
          </div>
        ...
        {isLoadingPages ? (
          <div class="flex flex-1 items-center justify-center text-sm text-text-muted">{t('library.loadingPages', 'Loading pages…')}</div>
        ) : filteredPages.length === 0 ? (
          <div class="flex flex-1 flex-col items-center justify-center gap-2 text-center text-text-muted">
            <p class="text-base font-semibold text-text-base">{t('library.noPagesFound', 'No pages found')}</p>
            <p class="text-sm">{t('library.noPagesFoundDescription', 'Create a new page from one of the starters above.')}</p>
          </div>
        ) : (
          <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            {filteredPages.map(page => (
              <PageCard key={page.id} page={page} onOpen={loadPost} onContextMenu={setContextMenu} />
            ))}
          </div>
        )}
      </section>
...
const PageCard: FunctionComponent<PageCardProps> = ({ page, onOpen, onContextMenu }) => (
  ...
  >
    <div class="flex w-full items-start justify-between gap-3">
      ...
      <span class={`rounded-badge px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wide ${statusClasses(page.status)}`}>
        {page.status}
      </span>
    </div>
    ...
    <div class="flex-1">
      ...
      <p class="mt-1 text-sm text-text-muted">{page.hasDocument ? t('library.containsBlockyDocument', 'Contains a Blocky document') : t('library.noBlockyDocument', 'WordPress page without a saved Blocky document')}</p>
    </div>

    <div class="flex w-full items-center justify-between gap-3 text-xs text-text-faint">
      <span>{formatModified(page.modified)}</span>
      <span class="font-semibold text-accent-text transition-transform group-hover:translate-x-0.5">{t('library.open', 'Open')}</span>
    </div>
  </button>
);
...
const PageContextMenu: FunctionComponent<PageContextMenuProps> = ({ page, x, y, onClose, onOpen, onDelete }) => {
  const actions = [
    {
      id: 'open',
    ...
    },
    {
      ...
      action: () => {
        onClose();
        if (!window.confirm(t('library.confirmTrash', 'Move "%s" to the trash?', [page.title]))) return;
  ...
  return (
    ...
    >
      <div class="border-b border-border-subtle px-3 py-2">
        ...
        <p class="mt-0.5 text-xs uppercase tracking-wide text-text-faint">{t('library.contextMenuHint', 'Right click: page actions')}</p>
      </div>
      ...
      <div class="grid gap-1 p-1">
        {actions.map(action => (
          <button
            ...
            class={`rounded-input px-3 py-2 text-left text-sm transition-colors ${action.danger
...
function formatModified(value: string): string {
  ...
  return t('library.updatedOn', 'Updated %s', [date.toLocaleDateString(builderLocale(), { day: '2-digit', month: 'short', year: 'numeric' })]);
}